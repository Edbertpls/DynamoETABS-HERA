...
namespace DynamoSAP.Assembly
{
    ...
    public class Bake
    {
        ...
        /// <summary>
        ///  Create or Update ETABS model from Dynamo Structural Model
        /// </summary>
        /// <param name="StructuralModel">Dynamo Structural Model</param>
        /// <param name="Bake">Set true to bake the model in ETABS</param>
        ...
        public static Dictionary<string, object> ToSAP(StructuralModel StructuralModel, bool Bake, string Units = "kip_ft_F", bool Delete = true)
        {
            if (Bake)
            {
                ...
                //double LengthSF = ETABSConnection.Utilities.UnitConversion(Units, fromUnit); // Length Conversion Factor
                double LengthSF = 1;
        ...
        private static void CreateorUpdateFrame(Frame f, ref cSapModel mySapModel, double SF, bool update)
        {
            ...
            if (!update) // Create new 
            {
                ...
                //1. Create Frame
                ETABSConnection.StructureMapper.CreateUpdateFrm(ref mySapModel, f.BaseCrv.StartPoint.X * SF,
                ...
                bool renamed = ETABSConnection.StructureMapper.ChangeNameSAPfrm(ref mySapModel, dummy, f.Label);
                if (!renamed)
                {
                    f.Label = dummy;
                }
            ...
            bool exists = ETABSConnection.StructureMapper.IsSectionExistsFrm(ref mySapModel, f.SecProp.SectName, ref error);
            if (error != string.Empty)
            {
                report.Add(error);
                error = string.Empty;
            ...
            {
                string MatProp = ETABSConnection.MaterialMapper.DynamoToSap(f.SecProp.MatProp);
                //Import new section property
                ETABSConnection.StructureMapper.ImportPropFrm(ref mySapModel, f.SecProp.SectName, MatProp, f.SecProp.SectCatalog, ref error);
                if (error != string.Empty)
                {
                    report.Add(error);
                    error = string.Empty;
            ...
            ETABSConnection.StructureMapper.SetSectionFrm(ref mySapModel, f.Label, f.SecProp.SectName, ref error);
            if (error != string.Empty)
            {
                report.Add(error);
                error = string.Empty;
            ...
            ETABSConnection.JustificationMapper.DynamoToSAPFrm(ref mySapModel, f.Label, f.Just, ref error); // TO DO: lateral and vertical justificaton
            if (error != string.Empty)
            {
                report.Add(error);
                error = string.Empty;
            ...
            // 4. Set Rotation
            ETABSConnection.JustificationMapper.SetRotationFrm(ref mySapModel, f.Label, f.Angle, ref error);
            if (error != string.Empty)
            {
                report.Add(error);
                error = string.Empty;
        ...
        private static void SetReleases(Frame frm, ref cSapModel mySapModel)
        {
            ...
            string error = string.Empty;
            ETABSConnection.ReleaseMapper.Set(ref mySapModel, frm.Label, iireleases, jjreleases, ref error);
        ...
        private static void SetLoads(Frame frm, ref cSapModel mySapModel)
        {
            foreach (var load in frm.Loads)
            {
                if (load.LoadType == "PointLoad")
                {
                    ...
                    ETABSConnection.LoadMapper.CreatePointLoad(ref mySapModel, frm.Label, load.lPattern.name, load.FMType, load.Dir, load.Dist, load.Val, load.CSys, load.RelDist, true);
                }
                if (load.LoadType == "DistributedLoad")
                {
                    //Call the CreateDistributedLoad method
                    ETABSConnection.LoadMapper.CreateDistributedLoad(ref mySapModel, frm.Label, load.lPattern.name, load.FMType, load.Dir, load.Dist, load.Dist2, load.Val, load.Val2, load.CSys, load.RelDist, true);
                }
            }
        }
        ...
        private static void CreateorUpdateArea(Shell s, ref cSapModel mySAPModel, double SF, bool update)
        {
            ...
            if (!update) // create new one
            {
                ...
                if (s.BaseMesh != null)
                {
                    ETABSConnection.StructureMapper.CreateorUpdateArea(ref mySapModel, s.BaseMesh, ref dummy, false, SF, ref error);
                    if (error != string.Empty)
                    {
                        report.Add(error);
                        error = string.Empty;
                ...
                {
                    ETABSConnection.StructureMapper.CreateorUpdateArea(ref mySapModel, s.BaseSurface, ref dummy, false, SF, ref error);
                    if (error != string.Empty)
                    {
                        report.Add(error);
                        error = string.Empty;
                ...
                // Set custom Label to Frame in dynamo & Frame! User can match by using Label & ID
                bool renamed = ETABSConnection.StructureMapper.ChangeNameSAPArea(ref mySapModel, dummy, s.Label);
                if (!renamed)
                {
                    s.Label = dummy;
                }
            ...
            else // modify the existing
            {
                ...
                if (s.BaseMesh != null)
                {
                    ETABSConnection.StructureMapper.CreateorUpdateArea(ref mySapModel, s.BaseMesh, ref id, true, SF, ref error);
                    if (error != string.Empty)
                    {
                        report.Add(error);
                        error = string.Empty;
                ...
                {
                    ETABSConnection.StructureMapper.CreateorUpdateArea(ref mySapModel, s.BaseSurface, ref id, true, SF, ref error);
                    if (error != string.Empty)
                    {
                        report.Add(error);
                        error = string.Empty;
            ...
            ETABSConnection.StructureMapper.SetPropArea(ref mySapModel,
            ...
            ETABSConnection.StructureMapper.SetShellPropArea(ref mySapModel, s.Label, s.shellProp.PropName, ref error);
            if (error != string.Empty) report.Add(error);
        }
        ...
        private static void CreateorUpdateJoint(Joint j, ref cSapModel mySAPModel, double SF, bool update)
        {
            if (!update) // create new one
            {
                ...
                // Set custom Label to Frame in dynamo & Frame! User can match by using Label & ID
                bool renamed = ETABSConnection.StructureMapper.ChangeNameSAPJoint(ref mySapModel, dummy, j.Label);
                if (!renamed)
                {
                    j.Label = dummy;
                }
            ...
            if (j.JointRestraint != null)
            {
                ...
                // Set restaints
                ETABSConnection.RestraintMapper.Set(ref mySapModel, j.Label, restraints.ToArray());
            }
        ...
        private static void CreateorUpdateGroup(Group g, ref cSapModel mySAPModel, bool update)
        {
            ...
            {
                ETABSConnection.GroupMapper.DefineGroup(ref mySapModel, g.Name);
            }
            else
            {
                // clear existing assignments
                ETABSConnection.GroupMapper.ClearGroupAssigment(ref mySapModel, g.Name);
            }
            ...
            try
            {
                ...
                ETABSConnection.GroupMapper.SetGroupAssign_Frm(ref mySapModel, g.Name, Frms);
            ...
            try
            {
                ...
                ETABSConnection.GroupMapper.SetGroupAssign_Shell(ref mySapModel, g.Name, Shells);
            ...
            try
            {
                ...
                ETABSConnection.GroupMapper.SetGroupAssign_Joint(ref mySapModel, g.Name, Joints);
        ...
        private static void CreateorUpdateSAPModel(ref StructuralModel StructuralModel, string Units, double SF, bool delete)
        {
            ...
            if (mySapModel == null)
            {
                ...
                try
                {
                    ETABSConnection.Initialize.InitializeSapModel(ref mySapObject, ref mySapModel, Units);
                }
                catch (Exception)
                {
                    ETABSConnection.Initialize.Release(ref mySapObject, ref mySapModel);
                };
            }

            //2. Create or Update Frames (Sets Releases)
            // 2.a. Harvest the elements from SAP Model
            ETABSConnection.StructureMapper.GetSAPFrameList(ref mySapModel, ref SAPFrmList); // frms
            ETABSConnection.StructureMapper.GetSAPAreaList(ref mySapModel, ref SAPAreaList); // areas
            ETABSConnection.StructureMapper.GetSAPJointList(ref mySapModel, ref SAPJointList); // joints
            ...
            if (delete)
            {
                ...
                foreach (var sapfrm in SAPFrmList)
                {
                    ...
                    {
                        ETABSConnection.StructureMapper.DeleteFrm(ref mySapModel, sapfrm);
                    }
                ...
                foreach (var sapArea in SAPAreaList)
                {
                    ...
                    //if (el == null)
                    //{
                    ETABSConnection.StructureMapper.DeleteArea(ref mySapModel, sapArea, ref error);
                    if (error != string.Empty)
                    {
                        report.Add(error);
                        error = string.Empty;
                ...
                foreach (var sapJoint in SAPJointList)
                {
                    ...
                    {
                        ETABSConnection.StructureMapper.DeleteJoint(ref mySapModel, sapJoint, ref error);
                        if (error != string.Empty)
                        {
                            report.Add(error);
                            error = string.Empty;
            ...
            if (LPatterns.Count > 0)
            {
                foreach (LoadPattern lp in LPatterns)
                {
                    ...
                    ETABSConnection.LoadMapper.AddLoadPattern(ref mySapModel, lp.name, lp.type, lp.multiplier);
                }
            }
            ...
            if (LCases.Count > 0)
            {
                foreach (LoadCase lc in LCases)
                {
                    ...
                    ETABSConnection.LoadMapper.AddLoadCase(ref mySapModel, lc.name, types.Count(), ref Dtypes, ref Dnames, ref DSFs, lc.type);
                }
            }
            ...
            if (LCombo.Count > 0)
            {
                foreach (LoadCombo lc in LCombo)
                {
                    ...
                    ETABSConnection.LoadMapper.AddLoadCombo(ref mySapModel, lc.name, Dtypes, Dnames, DSFs, lc.type);
                }
            }
            ...
            ETABSConnection.GroupMapper.GetSAPGroupList(ref mySapModel, ref SAPGroupList);
            ...
            if (delete)
            {
                foreach (var g in SAPGroupList)
                {
                    ...
                    {
                        ETABSConnection.GroupMapper.Delete(ref mySapModel, g);
                    }
            ...
            // refresh View 
            //ETABSConnection.StructureMapper.RefreshView(ref mySapModel);

            // Delete unconnected points at the SAP
            ETABSConnection.StructureMapper.DeleteUnconnectedPts(ref mySapModel);
